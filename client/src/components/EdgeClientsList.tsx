...
interface EdgeClient {
  id: string;
  name: string;
  status: 'online' | 'offline' | 'participating';
  lastSeen: string;
  capabilities: {
    video?: boolean;
    audio?: boolean;
...
export function EdgeClientsList() {
      ...
      <Card className="glass-morphism">
        ...
        <CardContent>
          <div className="text-center py-8">
            <p className="text-red-400">Error loading devices data</p>
          </div>
        </CardContent>
      </Card>
    );
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'participating': return 'bg-blue-500';
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'participating': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 border-blue-200 dark:border-blue-800';
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              { key: 'participating', label: 'Participating', count: clients?.filter(c => c.status === 'participating').length || 0 },
      ...
      <CardContent className="p-6 pt-0">
        <div className={`grid gap-4 ${
          ...
          {filteredClients.map((client) => (
            <Card key={client.id} className="bg-white/80 dark:bg-slate-800/80 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02] group">
              <div className="relative z-10 p-4">
                ...
                <div className="space-y-3 mb-4">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className={`font-bold ${client.status === 'online' ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  {client.uptime !== undefined && (
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-medium text-gray-800 dark:text-gray-200">{client.uptime.toFixed(1)}%</span>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-gray-100 dark:bg-slate-700/50 rounded-lg p-3 mb-4">
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {client.capabilities.video && (
                      <div className="flex items-center space-x-2">
                        <Eye className="w-3 h-3 text-blue-500" />
                        <span className="text-gray-600 dark:text-gray-300">Video</span>
                      </div>
                    )}
                    {client.capabilities.audio && (
                      <div className="flex items-center space-x-2">
                        <Cpu className="w-3 h-3 text-green-500" />
                        <span className="text-gray-600 dark:text-gray-300">Audio</span>
                      </div>
                    )}
                    {client.capabilities.nightVision && (
                      <div className="flex items-center space-x-2">
                        <Shield className="w-3 h-3 text-purple-500" />
                        <span className="text-gray-600 dark:text-gray-300">Night Vision</span>
                      </div>
                    )}
                    {client.securityLevel && (
                      <div className="flex items-center space-x-2">
                        <Shield className="w-3 h-3 text-amber-500" />
                        <span className="text-gray-600 dark:text-gray-300">{client.securityLevel}</span>
                      </div>
                    )}
                  </div>
                </div>
